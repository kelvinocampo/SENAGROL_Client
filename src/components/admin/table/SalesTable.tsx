/* -----------------------------------------------------------
   SalesTable.tsx
   Reemplaza <SearchBar /> por tu componente <Buscador />
...
import Buscador from "@/components/Inicio/Search";            // ✅ Usa Buscador
...
export const SalesTable = () => {
  ...
  const filteredSales = sales.filter((sale) => {
    ...
    const saleMonth = sale.fecha_compra?.slice(5, 7); // "YYYY-MM-DD"
  ...
  <div className="w-full md:w-auto flex-1">
    <Buscador
      ...
      setPaginaActual={() => {}}
      placeholderText="Buscar vendedor, comprador, estado o precio..."
    />
  </div>
          ...
          {filteredSales.length === 0 ? (
            ...
            filteredSales.map((sale) => (
              <motion.tr
                ...
                layout
              >
                <td className="p-2 text-left break-words max-w-[180px]">
                ...
                <td className="p-2 break-words max-w-[140px]">{sale.precio_producto}</td>
                <td className="p-2 break-words max-w-[140px]">{sale.estado}</td>
              </motion.tr>
            ))
          )}
        </tbody>
