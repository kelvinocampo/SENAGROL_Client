  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const comprasFiltradas = data.filter(
          (compra: any) => compra.estado === "En Proceso" || compra.estado === "Completada"
        );
  ...
  return (
    <div className="flex-1 px-6 py-4">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex gap-4 items-center">
          ...
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 bg-yellow-400 rounded-full" /> <span>P</span>
            <span className="w-3 h-3 bg-green-500 rounded-full" /> <span>C</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4 mt-20   ">
        {compras.map((compra) => (
          <div key={compra.id_compra} className="flex items-start gap-4 border-b pb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-700">
                {compra.estado}, Circassia<br />
                <span className="text-green-600 font-bold">${compra.precio_transporte}</span><br />
                Cant: {compra.cantidad}<br />
                Vendedor: {compra.vendedor_nombre}<br />
                Entregado el {new Date(compra.fecha_entrega).toLocaleDateString()}
              </p>
            </div>
